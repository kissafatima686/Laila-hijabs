...
const btnGray = { padding: '7px 12px', borderRadius: '6px', backgroundColor: '#4a5542', border: '1px solid #7d8b6b', color: '#F6F1E3', fontSize: '12px', cursor: 'pointer' };
...
const BundleCard = ({ bundle, idx, removeBundle, handleBundleChange }) => {
    ...
    return (
      <div style={{ backgroundColor: '#182012', borderRadius: '12px', padding: '16px', border: '1px solid rgba(184,147,91,0.3)', display: 'flex', flexDirection: 'column', gap: '10px', opacity: isActive ? 1 : 0.6 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '11px', color: '#B8935B', fontWeight: '700' }}>BUNDLE #{idx + 1} {isActive ? '' : '(DEACTIVATED)'}</span>
          <div style={{ display: 'flex', gap: '8px' }}>
             <button type="button" onClick={toggleStatus} style={btnGray}>{isActive ? 'Deactivate' : 'Activate'}</button>
             <button type="button" onClick={() => setIsEditing(true)} style={btnG}>Edit</button>
  ...
  return (
    <div style={{ backgroundColor: '#182012', borderRadius: '12px', padding: '16px', border: '1px solid #B8935B', display: 'flex', flexDirection: 'column', gap: '10px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: '11px', color: '#B8935B', fontWeight: '700' }}>EDITING BUNDLE #{idx + 1}</span>
        <button type="button" onClick={() => setIsEditing(false)} style={btnP}>Done Editing</button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
        <div>
          ...
          <input value={bundle.bundle_price} onChange={e => handleBundleChange(idx, 'bundle_price', e.target.value)} style={{ ...iStyle, color: '#B8935B', fontWeight: '800' }} placeholder="Rs. 5,500" />
        </div>
        <div>
          <label style={lStyle}>Original Price</label>
          <input value={bundle.original_price} onChange={e => handleBundleChange(idx, 'original_price', e.target.value)} style={iStyle} placeholder="Rs. 7,500" />
        </div>
        <div>
          <label style={lStyle}>Savings Badge</label>
          <input value={bundle.savings} onChange={e => handleBundleChange(idx, 'savings', e.target.value)} style={{ ...iStyle, color: '#22c55e', fontWeight: '700' }} placeholder="SAVE 26%" />
        </div>
      </div>

      <div>
        <label style={lStyle}>Items Included</label>
        <input value={bundle.items_included} onChange={e => handleBundleChange(idx, 'items_included', e.target.value)} style={iStyle} placeholder="3x Jersey Hijabs (Black, Nude, Taupe)" />
      </div>
...
const WayToSaveCard = ({ wc, idx, setForm, form }) => {
    ...
    const toggleStatus = () => {
        ...
        copy[idx].status = isActive ? 'Draft' : 'Live';
    ...
    if (!isEditing) {
        return (
            <div style={{ backgroundColor: '#182012', borderRadius: '12px', padding: '16px', border: '1px solid rgba(184,147,91,0.3)', display: 'flex', flexDirection: 'column', gap: '10px', opacity: isActive ? 1 : 0.6 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '10px', color: '#B8935B', fontWeight: '800' }}>CARD #{idx + 1} {isActive ? '' : '(DEACTIVATED)'}</span>
...
const OffersBundlesPageManager = () => {
  ...
  const [form, setForm] = useState({
    ...
    ways_cards: [
      { badge: 'FIRST ORDER', title: '10% Off Your First Purchase', description: 'New to Laila?', image_url: '/hero1.png', status: 'Live', slug: '' },
      { badge: 'BUNDLE', title: 'Buy 2 Hijabs, Save 15%', description: 'Mix and match...', image_url: '/hero2.png', status: 'Live', slug: '' },
      { badge: 'SPECIAL EDIT', title: 'Exclusive Eid Collection Offer', description: 'Explore luxury abayas...', image_url: '/hero3.png', status: 'Live', slug: '' }
    ],

    bundles: [
      { id: 'b1', title: 'Everyday Grace 3-Hijab Bundle', slug: 'everyday-grace-3-hijab-bundle', original_price: 'Rs. 7,500', bundle_price: 'Rs. 5,500', savings: 'SAVE 26%', image_url: '/hero1.png', items_included: '3x Jersey Hijabs (Black, Nude, Taupe)', status: 'Live' }
    ]
  });
      ...
      .then(d => {
        if (d && d.title) {
          ...
          setForm(prev => ({
            ...
            ways_tag: meta.ways_tag || prev.ways_tag,
            ways_title: meta.ways_title || prev.ways_title,
            ways_cards: meta.ways_cards || prev.ways_cards
          }));
        }
      })
      .catch(() => {})
    ...
    fetch(`${API}/sections/offers_bundles_page`, {
      ...
      body: JSON.stringify({
        ...
        metadata: {
          ...
          ways_tag: form.ways_tag,
          ways_title: form.ways_title,
          ways_cards: form.ways_cards
        }
      })
    })
      .then(() => {
      ...
      <div style={{ background: 'linear-gradient(135deg, #3E4930 0%, #222C1A 100%)', borderRadius: '16px', padding: '26px 30px', border: '1px solid #B8935B', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '14px' }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: '13px', color: '#E7D9C9' }}>
            Manage curated bundle packages, special bundle prices, savings badges, and product slugs.
          </p>
        </div>
          ...
          <div style={{ marginBottom: '26px', borderBottom: '1px solid rgba(184,147,91,0.2)', paddingBottom: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div>
                ...
                <h3 style={{ margin: '2px 0 0 0', fontSize: '17px', fontWeight: '800', color: '#F6F1E3' }}>Bundle Cards</h3>
              </div>
